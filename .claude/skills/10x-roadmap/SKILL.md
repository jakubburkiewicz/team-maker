...
Cykl życia kamienia milowego (stany, reguły wykrywania, przejścia, niezmienniki) jest określony w **`references/milestone-state.md`**, celowo pozostawionym poza tym plikiem. **Czytaj go tylko wtedy, gdy wywołanie działa na poziomie kamienia milowego** — pierwsze uruchomienie, wznowienie/sprawdzenie statusu, zamknięcie kamienia milowego lub otwarcie następnego kamienia milowego. Czysta ponowna dekompozycja już otwartego kamienia milowego nie wymaga tego.
...
Bloki pytań pojawiają się w Krokach 1, 3, 4, 5 i 9 oraz w przejściach kamieni milowych w `references/milestone-state.md` — krótkie, ustrukturyzowane wybory. Krok 5 pyta o każdy kotwicę jako własne ustrukturyzowane pytanie; jego podsumowanie syntezy jest zwykłym markdownem (bez dodatkowego pytania).
...
Po uzyskaniu 2-3 odpowiedzi na pytania kotwiczące, wyprowadź obszary inwestycji z: (1) wybranego `main_goal`, (2) NFR w PRD blokujących uruchomienie w warstwie, (3) luk w bazowej mapowanych na FR typu must-have, (4) koncentracji otwartych pytań. Ogłoś wyprowadzoną inwestycję w podsumowaniu syntezy (5e). Użytkownik może nadpisać w jednej linii; nie jest proszony o wybór.
...
- **Krok 5 "Gdzie inwestować"** — wybory "inwestuj głęboko" promują fundament do własnego, jawnego fragmentu (np. "warstwa danych — inwestuj głęboko" + brak bazowej → F-NN jawny fundament projektowania danych, a nie tylko niejawny krok migracji).
...
Każdy `F-NN` i `S-NN` otrzymuje również stabilny **Change ID** w kebab-case. Change ID jest pomostem do `/10x-plan`, a później elementem backlogu w Jira/Linear. Preferuj zwięzłe, zorientowane na wynik nazwy, takie jak `first-gated-generation`, `minimal-auth-for-generation` lub `srs-review-session`.
...
- **Kotwice zakresu:** <Identyfikatory PRD, z których ten kamień milowy czerpie (zakresy FR-NNN, US-NN) — lub, dla kamieni milowych pochodzących z opisu, numerowane elementy `MS-NN` destylowane dosłownie z opisu użytkownika:>
...
Pomoc nawigacyjna — grupuje elementy, które współdzielą łańcuch wymagań wstępnych. Kanoniczna kolejność nadal znajduje się w poniższym grafie zależności; ta tabela to proponowana kolejność czytania w równoległych ścieżkach.
...
- **Odniesienia do PRD** używają dosłownych identyfikatorów z PRD (`FR-001`, `US-01`, `NFR-02`). Nie parafrazuj. Każdy FR typu must-have w PRD musi pojawić się w co najmniej jednym fragmencie w sekcji Odniesienia do PRD po samokontroli w Kroku 8.
...
- **Niewiadome** to pytania do zbadania. Każde zawiera Właściciela i flagę Blokowania. Blokowanie=tak zmienia status fragmentu na `blocked`.
...
4. **Pokrycie PRD** — każdy FR typu `must-have` z PRD (grep `^- FR-\d{3}: .* must-have$`) pojawia się w co najmniej jednym fragmencie w sekcji `PRD refs`. To samo dotyczy każdego `### US-NN:`. Jeśli must-have nie jest pokryte, samokontrola NIE POWODZI SIĘ.
...
16. **Integralność kamienia milowego** — `milestone_status` jest `open` podczas generowania; `milestone_seq` jest o 1 większe niż najwyższe zamknięte `M-<seq>` w `## Milestone History` (1, gdy historia jest pusta); karta `## Milestone` `M-<seq>` pasuje do `milestone_seq`; każda kotwica `MS-NN` odwołująca się do dowolnego fragmentu istnieje w karcie; `## Milestone History` została przeniesiona dosłownie (nigdy nie edytowana, nigdy nie obcinana) podczas regeneracji i otwierania następnego kamienia milowego.
...
Jeśli plik istnieje, konwencja dokumentu bazowego to **edycja na miejscu** dla stopniowego udoskonalania, **archiwizacja, a następnie zastąpienie** dla pełnej regeneracji. Ta umiejętność tworzy *pełną* mapę drogową z PRD; chirurgiczne udoskonalanie jest poza zakresem. Domyślnie więc archiwizuj, a następnie zastąp, ale zapytaj za pomocą wybranego narzędzia do pytań interaktywnych:
...
5. **Brak szacunków, brak jednostek czasu.** Brak "Dzień 1", brak "2 tygodnie", brak "mały/średni/duży", brak punktów. Wykonanie agenta AI jest nieliniowe, a szacunki budżetowane czasowo są fałszywe. Kolejność jest zakodowana w wymaganiach wstępnych; tempo jest ujawniane poprzez blokery i niewiadome. Mapa drogowa opisuje kształt, a nie harmonogram.
...
- Ta umiejętność to **generator dokumentów plus tracker kamieni milowych**. Wynikiem jest `context/foundation/roadmap.md`, kropka. Planowanie poszczególnych zmian odbywa się w dół strumienia w `/10x-plan`.
