...
4. **Przygotuj znacznik do zmiany nazwy.** Edycja w kroku 2 zmodyfikowała `change.md` w drzewie roboczym, ale `git mv` tylko przygotowuje zmianę nazwy z zawartością HEAD pliku. Uruchom `git add "$DEST/change.md"`, aby znacznik frontmatter trafił do tego samego zatwierdzenia co zmiana nazwy.

5. **Zamknij pasujący element roadmapy.** Uruchom to przy **każdej** archiwizacji — wyszukiwanie roadmapy jest obowiązkowe. „Najlepszy wysiłek” obejmuje tylko *edycje*: brakująca roadmapa lub nieznaleziony cel edycji jest cicho pomijany i nigdy nie blokuje, nie wycofuje ani nie monituje o archiwizację. NIE oznacza to „założenia, że nie ma roadmapy i pominięcia sprawdzenia”. Niepatrzenie jest wadą — potwierdzenie (krok 7) musi zgłosić wynik tak czy inaczej.
...
6. **Zatwierdź archiwum.** Utwórz jedno zatwierdzenie:
   ...
   Bez treści — temat jest mechaniczny, a różnica (zmiana nazwy + znacznik frontmatter, plus zamknięcie roadmapy, gdy pasowało) jest oczywista. Nigdy nie przekazuj flag `--no-verify` ani flag pomijających podpisywanie. Jeśli hook pre-commit zawiedzie, napraw podstawowy problem i utwórz NOWE zatwierdzenie.
