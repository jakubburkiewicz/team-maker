...
- NIE badaj ponownie ram. Ramka odpowiada za ujęcie problemu; Ty odpowiadasz za projektowanie rozwiązania.

**Gdy badanie jest obecne**, przeczytaj je W CAŁOŚCI i użyj jako podstawy bazy kodu:
...
1. **Natychmiast i W CAŁOŚCI przeczytaj wszystkie wymienione pliki**:
   ...
   - `context/foundation/lessons.md` jeśli jest obecny — traktuj jego zasady jako priorytety podczas badania zakresu, przypadków brzegowych i wyborów architektonicznych; zasady już zaakceptowane przez zespół zawężają, które pułapki projektowe nadal wymagają świeżego kwestionowania.
...
5. **Przedstaw świadome zrozumienie i oceń złożoność**:
   ...
   **Skala złożoności:**
   ...
   | **ŚREDNI** | 7-10      | Wiele komponentów lub rozważań, które wchodzą w interakcje. Wymaga decyzji projektowych, ma przypadki brzegowe warte omówienia, pewna niejednoznaczność w podejściu. Przykłady oprogramowania: funkcja wieloplikowa, nowy punkt końcowy API. Przykłady nie-oprogramowania: wieloczęściowy plan treści, przeprojektowanie przepływu pracy, moduł kursu.                               |
...
6. **Zadawaj głębokie, dociekliwe pytania za pomocą AskUserQuestion**:
   ...
   **Każda opcja MUSI zawierać sygnał rekomendacji i analizę kompromisów:**
   ...
   - Rekomendacja powinna być oparta na badaniach (wzorcach bazy kodu dla oprogramowania, wiedzy dziedzinowej i kontekście dla zadań nie-oprogramowania) — a nie na zgadywaniu
   ...
   **O co pytać** — dostosuj kategorie do dziedziny zadania:
   ...
   Najpierw zidentyfikuj dziedzinę zadania: **oprogramowanie**, **treść/edukacja**, **strategia/proces** lub **hybryda**. Następnie wybierz odpowiednie kategorie pytań. Poniższe kategorie są uporządkowane według dziedziny — wybierz to, co istotne, nie narzucaj kategorii oprogramowania zadaniom nie-oprogramowania.
   ...
   **O co NIE pytać:**
   ...
   - Niskopoziomowe szczegóły implementacji, które możesz ustalić samodzielnie (na podstawie badań bazy kodu dla oprogramowania, na podstawie plików kontekstowych i wcześniejszych prac dla zadań nie-oprogramowania)
...
Jeśli żadne z nich nie ma zastosowania, pomiń całą sekcję. Plan bez niej nie jest niekompletny; plan, który wypełnia ją szablonowymi punktorami, jest nadmierny.
...
**Uwaga implementacyjna**: Po zakończeniu tej fazy i wszystkich automatycznych testach weryfikacyjnych, zatrzymaj się tutaj w celu ręcznego potwierdzenia przez człowieka, że testy ręczne zakończyły się sukcesem, zanim przejdziesz do następnej fazy. Bloki faz używają zwykłych punktorów — odpowiadające im pola wyboru `- [ ]` dla tych elementów znajdują się w sekcji `## Progress` na dole planu.
...
> Krótki opis ramowy: `context/changes/<change-id>/frame.md` (jeśli jest obecny — w przeciwnym razie pomiń linię)
> Badania: `context/changes/<change-id>/research.md` (jeśli jest obecny — w przeciwnym razie pomiń linię)
...
3. **Kluczowe zasady briefu**:
   ...
   - Pisz dla kogoś, kto nie brał udziału w rozmowie planistycznej — powinien zrozumieć kształt i uzasadnienie planu na podstawie samego briefu.
...
- question: "Jakie jest główne wąskie gardło w obecnym procesie newslettera?"
  ...
  - label: "Dystrybucja i harmonogramowanie"
    description: "Treść jest gotowa, ale publikowanie jest ręczne. · Mocna strona: Najłatwiejsze do zautomatyzowania — jasne wejścia i wyjścia. · Kompromis: Najmniejszy wpływ, jeśli kuracja lub pisanie nadal jest wąskim gardłem."
    multiSelect: false
