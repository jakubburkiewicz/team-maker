...
- The test runner is Vitest: `npm test` runs `vitest run` (single pass, no watch) over `src/**/*.test.ts`. Tests run in plain Node — no Astro runtime, no Supabase stack, no secrets, no Docker; CI runs `npm test` without any of them. The constraint is on what **evaluates** at test time, not on what a source line says. Nothing may load a module that resolves an `astro:*` virtual module (only `src/lib/supabase.ts` and `src/lib/config-status.ts` do, plus `astro:middleware` in `src/middleware.ts`) or that constructs a real Supabase client. What follows: `import type { APIRoute } from "astro"` is fine — type imports are erased at transpile; `node:fs` is fine (`src/lib/teams-policy-sql.test.ts` reads migrations that way); and a file that imports `@/lib/supabase` — an API route, say — **can** be under test as long as the test replaces that module, since `vi.mock("@/lib/supabase", …)` is hoisted and the real one never evaluates. Never build a real client in a test: pass a fake as the argument the `src/lib/` data modules already take. `zod` is still not a dependency; do not add zod validation unless asked.
...
- Local dev is `npm run dev` — Astro 6 runs the real `workerd` runtime through the Cloudflare Vite plugin. Do not use `wrangler dev`. Local secrets live in `.env` (gitignored; wrangler 4.90 loads it the same way it loads `.dev.vars`), production secrets in `npx wrangler secret put <NAME>`. E2E (`npx playwright test`) runs **only** against the local Supabase stack, and the switch is the content of `.env` — never create a root `.dev.vars`, which **disables** `.env` rather than supplementing it; `npm run preview`, which `webServer` starts, serves the values **frozen at build time** in `dist/server/.dev.vars`, so the order is `.env` → `npm run build` → `npx playwright test` (`e2e/stack-guard.ts` refuses every break of it).
...
- Protect a route by adding its path to `isProtectedRoute()` in `src/lib/routes.ts` — an **exact** entry (matched by equality, which is how `/` is protected without swallowing `/auth/signin`) or a **prefix** entry (matched on a segment boundary, covering subroutes). `src/middleware.ts` enforces it.
